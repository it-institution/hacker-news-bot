...
if discord_webhook_url is None and slack_webhook_url is None:
    raise ValueError("Both DISCORD_WEBHOOK_URL and SLACK_WEBHOOK_URL are not set.")
...
def get_db_connection():
    ...
    conn.execute(
        """CREATE TABLE IF NOT EXISTS posted_stories (story_id TEXT, platform TEXT, PRIMARY KEY (story_id, platform))"""
    )
    return conn
...
def save_posted_story_db(platform, story_id):
    with get_db_connection() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO posted_stories (story_id, platform) VALUES (?, ?)",
            (story_id, platform),
        )
...
def is_story_posted_db(platform, story_id):
    with get_db_connection() as conn:
        result = conn.execute(
            "SELECT story_id FROM posted_stories WHERE story_id = ? AND platform = ?",
            (story_id, platform),
        ).fetchone()
...
def save_posted_story(platform, story_id):
    if storage_type == "database":
        ...
        file_path = (
            DISCORD_POSTED_STORIES_FILE
            if platform == "discord"
            else SLACK_POSTED_STORIES_FILE
        )
    ...
    for platform, webhook_url in (
        ...
        for story_id in requests.get(HN_TOP_STORIES_URL).json():
            ...
            if story and "url" in story:
                post_title = f"HN | {story['title']}"
                if platform == "discord":
                    message = f"**[{post_title}]({story['url']})**"
                else:  # slack
                    message = f"*<{story['url']}|{post_title}>*"
